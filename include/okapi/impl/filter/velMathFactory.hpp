...
public:
	 ...
	 */
	static VelMath
	create(double iticksPerRev, QTime isampleTime = 0_ms,
	 ...
	 */
	static VelMath
	create(double iticksPerRev, std::unique_ptr<Filter> ifilter,
	 ...
	 */
	static std::unique_ptr<VelMath> createPtr(
	    double iticksPerRev, std::unique_ptr<Filter> ifilter,
	    QTime isampleTime = 0_ms,
	    const std::shared_ptr<Logger>& ilogger = Logger::getDefaultLogger());
};
} // namespace okapi
