...
public:
	 ...
	 * ```
	 *
	 * @param iports The ports the potentiometer is plugged in to in the order
	 * ``{smart port, potentiometer port}``. The smart port is the smart port
	 * number (``[1, 21]``). The potentiometer port is the ADI port number (``[1,
	 * 8]``, ``[a, h]``, ``[A, H]``).
	 */
