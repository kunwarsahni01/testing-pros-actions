...
public:
	 ...
	 * ```
	 *
	 * Note to developers: Keep the default value on imultiplier so that users get
	 * an error if they do ADIGyro({1, 'A'}). Without it, this calls the
	 * non-ext-adi constructor.
	 *
	 * @param iports The ports the gyro is plugged in to in the order ``{smart
	 * port, gyro port}``. The smart port is the smart port number (``[1, 21]``).
	 * The gyro port is the ADI port number (``[1, 8]``, ``[a, h]``, ``[A, H]``).
	 * @param imultiplier A value multiplied by the gyro heading value.
	 */
