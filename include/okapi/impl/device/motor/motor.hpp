...
public:
	 ...
	 * A V5 motor.
	 *
	 * @param iport The port number in the range ``[1, 21]``. A negative port
	 * number is shorthand for reversing the motor.
	 */
	 ...
	 * @param ireverse Whether the motor is reversed (this setting is not written
	 * to the motor, it is maintained by okapi::Motor instead).
	 ...
	 *
	 * @note This function simply sets the target for the motor, it does not block
	 ...
	 *
	 * @return The motor's absolute position in its encoder units or PROS_ERR_F if
	 ...
	 *
	 * @param timestamp A pointer to a time in milliseconds for which the encoder
	...
	/**
	 * Sets the reverse flag for the motor. This will invert its movements and the
	 * values returned for its position.
	 *
	 ...
	 * failed, setting errno.
	 */
	virtual std::int32_t setPosPID(double ikF, double ikP, double ikI,
	                               double ikD);
	 ...
	 * @param ithreshold the threshold for determining if a position movement has
	 * reached its goal
	 * @param iloopSpeed the rate at which the PID computation is run (in ms)
	 * @return 1 if the operation was successful or PROS_ERR if the operation
	 * failed, setting errno.
	 */
	virtual std::int32_t setPosPIDFull(double ikF, double ikP, double ikI,
	                                   double ikD, double ifilter, double ilimit,
	                                   double ithreshold, double iloopSpeed);
	 ...
	 * failed, setting errno.
	 */
	virtual std::int32_t setVelPID(double ikF, double ikP, double ikI,
	                               double ikD);
	 ...
	 * failed, setting errno.
	 */
	virtual std::int32_t setVelPIDFull(double ikF, double ikP, double ikI,
	                                   double ikD, double ifilter, double ilimit,
	                                   double ithreshold, double iloopSpeed);
