...
public:
	 ...
	 * StateMode is `StateMode::FRAME_TRANSFORMATION`.
	 *
	...
	OdomChassisController(
	    TimeUtil itimeUtil, std::shared_ptr<Odometry> iodometry,
	 ...
	 */
	virtual void driveToPoint(const Point& ipoint, bool ibackwards = false,
	...
	/**
	 * Sets a default StateMode that will be used to interpret target points and
	 * query the Odometry state.
	 *
	...
	/**
	 * Set a new move threshold. Any requested movements smaller than this
	...
	/**
	 * Set a new turn threshold. Any requested turns smaller than this threshold
	 * will be skipped.
	 *
	...
	/**
	 * Starts the internal odometry thread. This should not be called by normal
	 * users.
	 */
